function TaskA_walkthroughGuessing_1(subjectnum)
...
%% test buttons
    ...
    Screen('Flip', window);
     ...
     %%%saving data info%%%
    outputname = fullfile(outputdir, [subjectnum '_TaskA_walkthroughGuessing_1.mat']);
    ...
    for k = 1:ntrials
        ...
        %%FIRST screen with comp and self choices; no counterbalance in this practice game. All comp on left and all self on right. 
        if trial_order(1,k) == 1
            ...
            Screen('TextStyle', window, 1);
            Screen('DrawText', window, msg, (centerhoriz-(normBoundsRect(3)*1.75)), centervert-220, black);
            Screen('TextStyle', window, 0);
            ...
            [normBoundsRect, offsetBoundsRect] = Screen('TextBounds', window, msg);
            Screen('TextStyle', window, 1);
            Screen('DrawText', window, msg, (centerhoriz+(normBoundsRect(3)/1)), centervert-220, black);
            Screen('TextStyle', window, 0);
        else 
            ...
            [normBoundsRect, offsetBoundsRect] = Screen('TextBounds', window, msg);
            Screen('TextStyle', window, 1);
            Screen('DrawText', window, msg, (centerhoriz-(normBoundsRect(3)*1.75)), centervert-220, black);    %centervert-(normBoundsRect(4)/2)
            Screen('TextStyle', window, 0); 
        ...
        while ~press
            ...
            if trial_order(1,k) == 1 
                if find(responsecode) == L_arrow %Left COMP
            ...
            elseif trial_order(1,k) == 2 
                if find(responsecode) == R_arrow %Right SELF
