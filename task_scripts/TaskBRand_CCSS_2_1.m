function TaskBRand_CCSS_2_1(subjectnum)
...
%% test buttons
    ...
    %%%saving data info%%%
    outputname = fullfile(outputdir, [subjectnum '_TaskBRand_CCSS_2_1.mat']);
    ...
    for k = trial_order(1,3):trial_order(end,3)
        ...
        Screen('TextStyle', window, 1);
        Screen('DrawText', window, msg1, (centerhoriz+(normBoundsRect(3)*0.65)), centervert-(normBoundsRect(4)/2), black);
        Screen('TextStyle', window, 0);
        ...
        Screen('TextStyle', window, 1);
        Screen('DrawText', window, msg2, (centerhoriz-(normBoundsRect(3)/0.62)), centervert-(normBoundsRect(4)/2), black);
        Screen('TextStyle', window, 0);
        ...
        while ~press
            ...
            if GetSecs - (compself_onset-slack) > responsewindow
                ...
                if find(responsecode) == R_arrow %Right varied  (old COMP)
                    ...
                    choicetracker(k,3) = variedmag;
                    choicetracker(k,4) = 0;
                elseif find(responsecode) == L_arrow %Left fixed (old SELF)
                    Choice_RT = GetSecs - compself_onset;
                    Choice_RT_onset = GetSecs - startsecs;                                                          
                    Screen('FillRect', window, chosen_color, LeftRect);                    
