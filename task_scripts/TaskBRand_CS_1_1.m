function TaskBRand_CS_1_1(subjectnum)
...
%% test buttons
    ...
    %%%saving data info%%%
    outputname = fullfile(outputdir, [subjectnum '_TaskBRand_CS_1_1.mat']);
    ...
    for k = 1:ntrials
        ...
        Screen('TextStyle', window, 1);
        Screen('DrawText', window, msg1, (centerhoriz-(normBoundsRect(3)*1.64)), centervert-(normBoundsRect(4)/2), black);
        Screen('TextStyle', window, 0);
        ...
        Screen('TextStyle', window, 1);
        Screen('DrawText', window, msg3, (centerhoriz+(normBoundsRect(3)/1.47)), centervert-(normBoundsRect(4)/2), black);
        Screen('TextStyle', window, 0);
        ...
        while ~press
            ...
            if GetSecs - (compself_onset-slack) > responsewindow
                ...
                if find(responsecode) == L_arrow %Left COMP
                    ...
                    comptracker(k,1) = compperc;
                    comptracker(k,3) = compmag;
                elseif find(responsecode) == R_arrow %Right SELF
                    Choice_RT = GetSecs - compself_onset;
                    Choice_RT_onset = GetSecs - startsecs;                    
                    Screen('FillRect', window, chosen_color, RightRect);                                        
